...
+------+----+-----------+--------+--------------+
*/
WITH weight_per_turn AS (
    SELECT
        person_name,
        SUM(weight) OVER (ORDER BY turn ASC) AS weight_per_turn
    FROM Queue
), people_in_bus AS (
    SELECT 
        person_name,
        weight_per_turn,
        ROW_NUMBER() OVER (ORDER BY weight_per_turn DESC) AS rank
    FROM weight_per_turn
    WHERE weight_per_turn <= 1000
)
SELECT 
    person_name
FROM people_in_bus 
