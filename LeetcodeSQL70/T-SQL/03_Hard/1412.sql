...
*/
WITH exams_results_ranked AS (
    SELECT
        e.exam_id,
        e.student_id,
        s.student_name,
        e.score,
        RANK() OVER (PARTITION BY e.exam_id ORDER BY e.score DESC) AS high_score_rank,
        RANK() OVER (PARTITION BY e.exam_id ORDER BY e.score ASC) AS low_score_rank
    FROM Exam e
    JOIN Student s
    ...
    SELECT
        student_id,
        student_name,
        MIN(high_score_rank) AS highest_score,
        MIN(low_score_rank) AS lowest_score
    FROM exams_results_ranked
